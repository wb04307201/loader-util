...
/**
 * Tab 4：DynamicRuntime 高级门面。把动态编译 + 类路径管理 + Bean 注册粘合到一次生命周期内。
 *
 * <p>每个 session 对应一个 {@link DynamicRuntime}（含独立 ClassLoader + 可选 BeanFactory）。
...
public class RuntimeController {
    ...
    public Map<String, Object> createSession() {
        DynamicRuntime rt = DynamicRuntime.withBeanFactory(bf());
    ...
    public ResponseEntity<Map<String, Object>> compile(@PathVariable String id,
        ...
        DynamicRuntime rt = registry.getRuntime(id);
        try {
