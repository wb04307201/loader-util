 ...
 * 关闭 {@link DynamoClassLoader} 以释放字节码缓存。
 *
 * <p>{@link DynamoRuntime} 由 Spring 容器在进程关闭时统一释放；这里只在删除
 * session 时调一次 {@link DynamoRuntime#close()}，让用户感知"session close"。
 */
@Component
public class InstanceRegistry {
    ...
    private final Map<String, DynamoClassLoader> compiledLoaders = new ConcurrentHashMap<>();
    ...
    private final Map<String, DynamoClassLoader> aspectLoaders = new ConcurrentHashMap<>();

    // Tab 4：DynamoRuntime session；session 内 register 的 controller 名字集合（list 时使用）
    private final Map<String, DynamoRuntime> runtimes = new ConcurrentHashMap<>();
    ...
    // Tab 3：直接通过 DynamoBean.registerController 注册的 controller（name → Class）
    ...
    public String putCompiled(Object instance, DynamoClassLoader loader) {
        String id = newId();
    ...
    public boolean removeCompiled(String id) {
        ...
        DynamoClassLoader loader = compiledLoaders.remove(id);
        if (loader != null) {
            loader.close();
        }
        return removed != null;
    ...
    public String putAspect(Object proxy, Deque<Map<String, Object>> log, DynamoClassLoader loader) {
        String id = newId();
    ...
    public boolean removeAspect(String id) {
        ...
        DynamoClassLoader loader = aspectLoaders.remove(id);
        if (loader != null) {
            loader.close();
        }
        return had;
    ...
    public String putRuntime(DynamoRuntime runtime) {
        String id = newId();
    ...
    public DynamoRuntime getRuntime(String id) {
        DynamoRuntime r = runtimes.get(id);
        if (r == null) {
    ...
    public boolean removeRuntime(String id) {
        DynamoRuntime r = runtimes.remove(id);
    ...
    // ---------- Tab 3：direct DynamoBean controller ----------
