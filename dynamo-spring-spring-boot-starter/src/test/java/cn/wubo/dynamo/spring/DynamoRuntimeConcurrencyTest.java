 ...
 * </ul>
 */
class DynamoRuntimeConcurrencyTest {
    ...
    void multipleInstances_isolatedClassLoaders() {
        // 两个 DynamoRuntime 实例编译同名类，应当加载到不同 ClassLoader。
        try (DynamoRuntime a = DynamoRuntime.create();
             DynamoRuntime b = DynamoRuntime.create()) {
    ...
    void close_concurrentWithCompile_isSafe() throws Exception {
        ...
        try {
            for (int t = 0; t < threads; t++) {
                final int id = t;
                futures.add(pool.submit(() -> {
                    try (DynamoRuntime r = DynamoRuntime.create()) {
                        start.await();
                        for (int i = 0; i < 50; i++) {
