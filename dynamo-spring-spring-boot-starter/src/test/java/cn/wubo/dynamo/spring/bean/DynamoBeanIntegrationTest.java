...
/**
 * DynamoBean + DynamoRequestMappingHandlerMapping 集成维度的单元测试。
 *
 ...
 */
class DynamoBeanIntegrationTest {
    ...
    void registerController_addsMappingToHandler() throws Exception {
        try (GenericApplicationContext ctx = new GenericApplicationContext()) {
            ...
            ctx.refresh();
            ctx.getBean(DynamoRequestMappingHandlerMapping.class); // 触发初始化
            ...
            try (DynamoClassLoader cl = DynamoClassLoader.create()) {
                ...
                DynamoBean.registerController(bf, "intCtrl", ctrl);
    ...
    void unregisterController_removesMapping() throws Exception {
        try (GenericApplicationContext ctx = new GenericApplicationContext()) {
            ...
            bf.registerBeanDefinition("mapping", mappingDef);
            ctx.refresh();
            ctx.getBean(DynamoRequestMappingHandlerMapping.class);
            ...
            try (DynamoClassLoader cl = DynamoClassLoader.create()) {
                ...
                DynamoBean.registerController(bf, "rmCtrl", ctrl);

                RequestMappingHandlerMapping mapping = ctx.getBean(RequestMappingHandlerMapping.class);
                assertThat(mapping.getHandlerMethods()).hasSize(1);

                DynamoBean.unregisterController(bf, "rmCtrl");
    ...
    void registerController_twiceWithDifferentTypes_appendsMapping() throws Exception {
        ...
        try (GenericApplicationContext ctx = new GenericApplicationContext()) {
            ...
            try (DynamoClassLoader cl = DynamoClassLoader.create()) {
                ...
                DynamoBean.registerController(bf, "shared", ctrl1);
                DynamoBean.registerController(bf, "shared", ctrl2);
    ...
    void registerController_afterUnregister_trulyReplaces() throws Exception {
        ...
        try (GenericApplicationContext ctx = new GenericApplicationContext()) {
            ...
            try (DynamoClassLoader cl = DynamoClassLoader.create()) {
                ...
                DynamoBean.registerController(bf, "repCtrl", ctrl1);
                DynamoBean.unregisterController(bf, "repCtrl");
                DynamoBean.registerController(bf, "repCtrl", ctrl2);
    ...
    void registerSingleton_thenGetBean_instantiates() {
        ...
        DynamoBean.registerSingleton(bf, "service", java.util.concurrent.atomic.AtomicLong.class);
    ...
    void unregisterSingleton_destroysInstance() {
        ...
        DynamoBean.unregisterSingleton(bf, "service");
