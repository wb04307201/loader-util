 ...
 *
 * <p>所有 controller 操作都依赖 Spring 容器中存在 {@link DynamoRequestMappingHandlerMapping} bean。
 * 启用 {@code DynamoBeanAutoConfiguration} 后该 bean 会自动注册。
 */
public final class DynamoBean {
    ...
    private static DynamoRequestMappingHandlerMapping getMapping(DefaultListableBeanFactory dbf) {
        try {
            return dbf.getBean(DynamoRequestMappingHandlerMapping.class);
        } catch (Exception e) {
            throw new IllegalStateException(
                "DynamoRequestMappingHandlerMapping bean not found. " +
                "Did you enable DynamoBeanAutoConfiguration?", e);
        }
    }
}
